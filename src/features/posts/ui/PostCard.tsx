...
} from "@/features/posts/queries/postLikes.queries.ts";
...
export function PostCard({ post, onShare, actions = [] }: PostCardProps) {
  ...
  const visibleActions = actions.filter(
    (action) => action.show === undefined || action.show(post),
  );
          ...
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center space-x-3">
              <Avatar className="ring-2 ring-white shadow-sm h-10 w-10">
                <AvatarImage
                  src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${post.createdBy}`}
                />

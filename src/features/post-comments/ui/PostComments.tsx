...
interface PostCommentsProps {
  ...
  showLikes?: boolean;
  readonly?: boolean;
  totalQuantityUpdated?: (total: number) => void;
}
...
export function PostComments({
  ...
  const totalCommentsQuantity = useMemo(() => {
    let quantity = initialTotalQuantity;

    if (data?.pagination?.total) {
      quantity = data?.pagination?.total;
    }

    return quantity;
  }, [initialTotalQuantity, data])
  ...
  if (!readonly) {
    canLoadMore = expanded ? hasNextPage : totalCommentsQuantity > PREVIEW_COUNT;
  }
    ...
    <div className="border-t border-border px-4 sm:px-5 pt-3 pb-4 space-y-3 bg-muted/30">
      {!readonly && (
        <div className="flex gap-2.5">
          <PostCommentAvatar user={currentUser} className="mt-0.5 shrink-0" fallbackClassName="gradient-brand" />
